...
DEFAULT_MIN_SIMILARITY: float = 0.865  # Minimum cosine similarity for an item to be considered a match
...
df1['sort_gp']= df1['company_name'].astype(str).str[0]
df1 = df1[df1['company_name'].notnull()]
df1.drop_duplicates(subset=['company_name'], keep='first', inplace=True)
#vals = df3['company_name'].to_numpy()
#df3['sortgp'] = df3['company_name'].astype(str).str[0]
unique_newsortgp = df1['sort_gp'].unique()

df2 = pd.read_sql(tablename,sqlite_engine, columns=['CompanyName'])
df2['sort_gp']= df2['CompanyName'].astype(str).str[0]
df2 = df2[df2['CompanyName'].notnull()]
df2.drop_duplicates(subset=['CompanyName'], keep='first', inplace=True)
unique_chsortgp = df2['sort_gp'].unique()

print(df1.shape, df2.shape)
print(df1.columns)
print(df2.columns)
results = pd.DataFrame()
for sortgp in unique_chsortgp:
    if sortgp in unique_newsortgp:
        ch_gp = df2.groupby(['sort_gp']).get_group(sortgp)
        ch = ch_gp['CompanyName']
        new_gp = df1.groupby(['sort_gp']).get_group(sortgp)
        new = new_gp['company_name']     
        matches = match_strings(ch, new)
        results = results.append(matches, ignore_index=True)
# Create all matches:


# Display the results:
#results= pd.DataFrame({'new_companies': new, 'matched': matches})
results.to_csv(directory1 + 'results.csv', index=False)
